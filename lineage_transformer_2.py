...
def _bare_table(table_ref: str) -> str:
    ...
    if not table_ref:
        return ""
    # Remove leading $VAR. prefix first
    s = re.sub(r"^\$\{?[A-Za-z_][A-Za-z0-9_]*\}?\.", "", table_ref.strip())
    # Then strip any remaining db.schema. qualifiers
...
def _extract_stmt_target_table(parsed: exp.Expression) -> str:
    ...
    try:
        ...
        if insert:
            ...
            if tbl:
                return _bare_table(tbl.name or "")

        # UPDATE
        update = parsed.find(exp.Update)
        if update:
            tbl = update.find(exp.Table)
            if tbl:
                return _bare_table(tbl.name or "")
...
def _resolve_source_alias(parsed: exp.Expression, source_table: str) -> str | None:
    ...
    Returns
    -------
    The alias string (upper-case) if found, else None.
    If the table appears without an alias, returns the bare table name itself.
    """
    if not source_table:
        return None
    try:
        for alias_node in parsed.find_all(exp.Alias):
            ...
            if isinstance(inner, exp.Table):
                if _bare_table(inner.name or "") == source_table:
                    return alias_node.alias.upper()
        # Also check plain Table nodes (no alias)
...
def process_single_file(task: dict[str, Any]) -> dict[str, Any]:
    ...
    for pair in pairs:
            ...
            for stmt in stmt_catalog:
                ...
                try:
                    ...
                    if found:
                        ...
                        if tgt_tbl and stmt_tgt == tgt_tbl:
        ...
        for stmt in parseable:
            stmt_tgt  = _extract_stmt_target_table(stmt["parsed"])
            tgt_match = (not tgt_tbl) or (stmt_tgt == tgt_tbl)
...
def run_pipeline(
    ...
    queries_df = (
        pd.DataFrame(all_queries)
          ...
          [["query_uuid", "sql_query", "file_path", "parse_status", "parse_error"]]
          .reset_index(drop=True)
    )
    n_failed = int((queries_df["parse_status"] == "PARSE_FAILED").sum())
    log.info(
